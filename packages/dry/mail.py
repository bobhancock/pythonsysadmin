...
def is_maillserver_avail(mailserver):
    """ True if mail server responds to ehlo """
...
def firs_maillserver_avail(mail_servers):
    """ Return the name of the first server in the list that
    that responds to an ehlo.  If none are available return None.	
    """
    server = None
    mail_servers
    for serv in mail_servers:
        s = smtplib.SMTP(serv)
        code = s.ehlo()[0]
        if (200 <= code <= 299):
            server = serv
...
def is_valid_email(email_address):
    """ Return True if email address has a valid format, otherwise False 

    N.B. This is not comprehensive, but will find mod errors.
    """
    email_str = email_address
    if ' ' in email_str:
        return False
    
    em = re.compile("^[a-zA-Z0-9._%&-]+@[a-zA-Z0-9._%-]+\.[a-zA-Z]{2,6}$")
    if em.match(email_str):
        return True
    else:
        return False
    

def setup_mailserver(mail_servers):
    """From a list of mail servers, return an smtp
    handle to the first one available. """
    mailserver = 'unassigned' # in case the next line fails

    if len(mail_servers) < 1:
        return None

    mailserver = firs_maillserver_avail(mail_servers)
